...
> - **D3 — Rubanova is CUT entirely** (Latent-ODE / ODE-RNN / sine / spiral removed from the paper scope and from `make reproduce-all`). Code is *not deleted* — it is excluded from the reproduction pipeline and gated tests and documented in `extra/README.md`. Rubanova stays only as a related-work citation.
...
- **Rubanova 2019: CUT** from the submission (kept in-repo under an `extra/` exclusion, related-work citation only). Its headline benchmarks (PhysioNet/MuJoCo/Human-Activity) are unreached and its paper has **no spiral** anyway (the 2-D spiral is Chen 2018's).
...
| **Rubanova 2019 (Latent ODE)** | **CUT (related-work only)** | nothing — Latent-ODE/ODE-RNN/sine/spiral code moved out of the reproduction pipeline into an `extra/` exclusion | any replication of Rubanova; that the 2-D spiral is a Rubanova experiment |
...
| C2 | **Backward NFE ≈ ½ forward NFE** (adjoint cheaper than direct backprop) | Fig 3c | trained ODE-Net, implicit-Adams adjoint | report Fig 12 shows backward ≈ **forward** ("near-lockstep") | ⚠️ **divergence → HEADLINE** | Our torchdiffeq adjoint gives backward≈forward, contradicting Chen's ½ | **PROMOTED.** (1) unit-test the fwd/bwd NFE split first — headline can't rest on a buggy counter; (2) dedicated sweep {solver: euler/rk4/bosh3/dopri5/dopri8} × {tol 1e-1…1e-6} × {arch: MLP, conv} × {≥5 seeds}, log fwd/bwd separately, report bwd/fwd ratio as primary quantity; (3) does the ratio ever approach 0.5? depend on solver order / tol / stiffness? (4) explain via torchdiffeq `odeint_adjoint` augmented reverse system (state+adjoint+param-sens solved jointly) vs Chen's implicit-Adams — read the source, don't speculate. If it's an instrumentation bug, report that loudly. | ~2–3 |
