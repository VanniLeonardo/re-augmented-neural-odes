...
| `rtqichen/ode-nets` (private, shared by the authors in September 2026) | Read, not used as code | The code and logs behind Chen et al. Fig. 3, sent by the authors in reply to our questions about claim 6. Read only after every model in this repository was written and every claim-6 experiment had finished. Nothing in our code derives from it, and it is not redistributed. The per-batch evaluation counts behind Fig. 3c, extracted from its logs by `scripts/chen_fig3c_audit.py`, are included with David Duvenaud's permission in `results/chen_fig3c/`. |
