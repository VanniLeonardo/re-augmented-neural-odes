...
| `c2` | Backward NFE is about half forward NFE (Fig. 3c) | Not reproduced. With code shared by the original authors, we traced the reported value of one half to a counting artefact, which they have confirmed; the corrected ratio is 0.78 to 0.86. Ours, with the adjoint at the forward tolerance as in the original, runs from 13 to 121 in the integrating regime, and falls sharply when the adjoint is solved at a looser tolerance. Neither that nor the error norm of the reverse solve accounts for the gap to the original's corrected value (`make c2-norm`). `make fig-c2` reports it as a tolerance and field surface, `scripts/c2_diagnosis_report.py` gives the diagnosis, and `scripts/chen_fig3c_audit.py` audits the original logs. | `make fig-c2` |
