...
| ID | Claim | Result | Figure |
|---|---|---|---|
| `c1` | Error falls and cost rises as the tolerance tightens, and time is proportional to NFE (Fig. 3a-b) | Mostly reproduced. Error falls monotonically for every adaptive solver, and Spearman ρ between time and NFE is 0.98 to 0.99. The monotone-cost condition recorded before the run is violated: dopri5 NFE falls from 26 to 20 between 1e-1 and 1e-2. Both of those rows fail the reconstruction check, so the violation lies where the solver is not integrating. The reference behind the error axis was itself checked: dopri8 at 1e-8 and at 1e-9 agree to 6e-6, under a tenth of 95% of the measured errors but 22% of the smallest (`make c1-reference`). | `make fig-c1` |
| `c2` | Backward NFE is about half forward NFE (Fig. 3c) | Not reproduced. With code shared by the original authors, we traced the reported value of one half to a counting artefact, which they have confirmed; the corrected ratio is 0.78 to 0.86. Ours, with the adjoint at the forward tolerance as in the original, runs from 13 to 121 on the two-dimensional fields and reaches 123 on the convolutional one, and falls sharply when the adjoint is solved at a looser tolerance. Neither that, nor the error norm of the reverse solve, nor the original's own solver accounts for the gap to the original's corrected value: VODE implicit Adams gives 109 where ours gives 128 on the same fields (`make c2-norm`, `make c2-vode`). `make fig-c2` reports it as a tolerance and field surface, `scripts/c2_diagnosis_report.py` gives the diagnosis, and `scripts/chen_fig3c_audit.py` audits the original logs. | `make fig-c2` |
...
- The grid reproduces the committed Figure 9 cells exactly, in all ten matched cells (five seeds × two models). Its median over ten seeds is 0.744 rather than the 0.619 of the five-seed headline, because two of the five added seeds are ones on which the NODE generalises perfectly.
