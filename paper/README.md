...
| Item | Where |
|---|---|
| Co-author name, ORCIDs, institutional emails, affiliation detail | `metadata.yaml` |
...
| **The repository has no `LICENSE` file** — ReScience expects openly licensed code | repository root |
| Confirm the `[Re]` vs `[¬Re]` title prefix with the editors if the undershoots are read as failure | `metadata.yaml` |
