...
$10^{-6}$. On the concentric spheres, the median forward NFE of the Neural ODE grows by a factor of \textbf{1.74}
between 25 and 500 epochs, the range of the condition we recorded in advance, and that of the
augmented model by \textbf{1.04}. At the original
...
growth factor of the Neural ODE is 1.37, which by coincidence equals the value for the circles
geometry above, so the effect holds without them.
...
values, so the cell that repeats it reads 0.744 rather than 0.619: two of the five added seeds
are ones on which the Neural ODE generalises perfectly. The augmented model generalises better at every width, on both
...
The reference itself is not exact, and we measured how inexact. On the same field its endpoint
...
adjoint directly, following the configuration of the code the authors shared: the augmented reverse system integrated
...
Claim 6 is the one we report as not reproduced. Before attributing the difference to the original, we measured its cause. The diagnosis of claim 6 tests three explanations against thresholds recorded in advance and
