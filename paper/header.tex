...
% LOCAL ADDITION to the upstream template: with more than three authors the class
% abbreviates the author list to "<first author> et al." in the page footer, the
% copyright line and the PDF metadata. The authors here are in alphabetical order
% and none of them leads the work, so all six are named in each place.
\def\authorsSHORT{Bassi, Ben Nefla, Grossi, Notaro, Porta and Vanni}
\def\authorsABBRV{G. Bassi, K. Ben Nefla, G. Grossi, A. Notaro, B. Porta and L. Vanni}
\def\authorsFULL{Giuseppe Bassi, Kabir Ben Nefla, Gaia Grossi, Anna Notaro, Beatrice Porta
  and Leonardo Vanni}
\hypersetup{pdfauthor={\authorsFULL}}
...
\begin{container}
  \afterpage {
    \begin{statement}
      ...
      % LOCAL FIX to the upstream template: each of the three branches below ended in
      % its own full stop, so a record with a DOI and no Software Heritage identifier
      % printed two. The full stop is now printed once, at the end.
      \ifdefempty{\codeURL}{}
...
%% --- Abstract ---------------------------------------------------------------
%% LOCAL CHANGE to the upstream template: this block ships commented out, so the
%% abstract from metadata.yaml is defined but never typeset. Published ReScience C
%% articles do show an abstract on page 1, so it is enabled here.
