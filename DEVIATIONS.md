...
| A2 | Toy vector field of width 32, `d+1 -> 32 -> 32 -> d` (App. F.1.1). | Width 32. | A wider field would let the Neural ODE approximate the distorted flow more easily, which flatters it. | matches |
...
| C3 | Adjoint memory is constant in the effective depth of the model (Table 1, memory column). | Varying the depth of a discrete baseline would test the wrong axis. `scripts/c4_memory_vs_nfe.py` holds the model fixed and raises its own NFE. | Adjoint memory is flat at +0.0007 MB per NFE, against +31.8 for direct backpropagation, over 3 seeds. | matches |
...
| # | The papers specify | This replication | Effect on the result | Status |
|---|---|---|---|---|
| D1 | Image batch size 256. Optimiser, learning rate and epoch counts are largely unstated in both papers. | Batch 256 for both image experiments. Adam at 1e-3. | The unstated epoch budgets are the most likely cause of our two accuracy undershoots, and we cannot attribute them cleanly. | matches |
