...
| A12 | Best d=2 **ANODE augmented dim = 5** (searched {1,2,5}, App. F.2.1). | Budget-sweep control uses **ANODE-p1** (p=1). | p=1 already gives the flat-NFE / higher-accuracy control cleanly. | p=1 suffices to show the ANODE signature (flat NFE ~180, recon 5/5, acc 0.999); p=5 would be even simpler/flatter. Not a threat to the NODE-vs-ANODE contrast. | **OPEN** — optionally add p=5 to match the paper's best. *(Updated 2026-09-10: the §6 grid (`results/slice_grid/`) includes p = 5 alongside p = 1, 2, 3. For held-out generalisation p = 2 is best at the widest wedge on both geometries and p = 5 does not beat p = 1–2; the NODE-vs-ANODE contrast holds for every p. Dupont chose p = 5 for fit on the full data, a different criterion, so this is characterisation, not a contradiction. No longer OPEN.)* |
